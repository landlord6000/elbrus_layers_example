...
// printf("B = %d X = %d Y = %d L = %d dXY = %f\n", B, X, Y, L, XY);
	// printf("in0 = %f in1 = %f in2 = %f in3 = %f\n", in[0], in[1], in[2], in[3]);
...
//---------------------------- cores
	...
	// 				pool_avg_global_core11(in + b*Y*X*L + y*X*L + x*L, out + b*L, L);
	// 			}
	// 			// printf("out0 = %f out1 = %f out2 = %f out3 = %f\n", *(out + b*L), *(out + b*L + 1), *(out + b*L + 2), *(out + b*L + 3));
	// 		pool_avg_global_last(out + b*L, XY, L);
	// 	}
	// }
	...
	// 		for(int y = 0; y < Y; y+=3)
	// 			for(int x = 0; x < X; x+=3) {
	// 				// pool_avg_global_core33(in + b*Y*X*L + y*X*L + x*L, out + b*L, X, L);
	// 				pool_avg_global_core33(in + b*Y*X*L + y*X*L + x*L, out + b*L, X, L);
	// 			}
	// 			// printf("out0 = %f out1 = %f out2 = %f out3 = %f\n", *(out + b*L), *(out + b*L + 1), *(out + b*L + 2), *(out + b*L + 3));
	// 		pool_avg_global_last(out + b*L, XY, L);
	...
	// 		for(int y = 0; y < Y; y+=5)
	// 			for(int x = 0; x < X; x+=5) {
	// 				pool_avg_global_core55(in + b*Y*X*L + y*X*L + x*L, out + b*L, X, L);
	// 				// pool_avg_global_core55_load(in + b*Y*X*L + y*X*L + x*L, out + b*L, X, L);
	// 			}
	// 			// printf("out0 = %f out1 = %f out2 = %f out3 = %f\n", *(out + b*L), *(out + b*L + 1), *(out + b*L + 2), *(out + b*L + 3));
	// 		pool_avg_global_last(out + b*L, XY, L);
	// 	}
...
//---------------------------- main func
	...
	// 			}	
	// 			x++;
	// 			pool_avg_global_core11_last(in + b*Y*X*L + y*X*L + x*L, out + b*L, out + b*L, XY, L);
	// 		}
	// 	}
	// 	else if(X == 3) {
	// 		long Yb = Y - (Y % 3);
	// 		for(int b = 0; b < B; ++b) {
	// 			for(int xy = 0; xy < X*Yb - 1; xy+=9) {
	// 				// x = xy % X;
	// 				y = xy / X;
	// 				printf("x = %d y = %d\n", x, y);
	// 				// pool_avg_global_core33(in + b*Y*X*L + y*X*L + x*L, out + b*L, L);
					
	// 				if((Y == Yb) && (y == Y - 3)) 
	// 					pool_avg_global_core33_last(in + b*Y*X*L + y*X*L, out + b*L, XY, X, L);	
	// 				else
	// 					pool_avg_global_core33(in + b*Y*X*L + y*X*L, out + b*L, X, L);
						
					
	// 			}	
	// 			y+=3;
	// 			for(long yt = y; yt < Y; ++yt) {
	// 				printf("x = %d yt = %d\n", x, yt);
	// 				pool_avg_global_core11(in + b*Y*X*L + yt*X*L, out + b*L, L);
	// 				pool_avg_global_core11(in + b*Y*X*L + yt*X*L + L, out + b*L, L);
	// 				if(yt < Y - 1) 
	// 					pool_avg_global_core11(in + b*Y*X*L + yt*X*L + 2*L, out + b*L, L);
	// 				else	
	// 					pool_avg_global_core11_last(in + b*Y*X*L + yt*X*L + 2*L, out + b*L, out + b*L, XY, L);
	...
	// 				if((X == Xb) && (x == X - 3)) 
	// 					pool_avg_global_core33_last(in + b*Y*X*L + x*L, out + b*L, XY, X, L);	
	// 				else
	// 					pool_avg_global_core33(in + b*Y*X*L + x*L, out + b*L, X, L);
	...
	// 			// 		pool_avg_global_core11(in + b*Y*X*L + yt*X*L + 2*L, out + b*L, L);
	// 			// 	else	
	// 			// 		pool_avg_global_core11_last(in + b*Y*X*L + yt*X*L + 2*L, out + b*L, XY, L);
	...
	// 		#pragma omp parallel
	// 		{
	// 			long x = 0, y = 0, xy = 0, b = 0; // private
	// 			int myid = omp_get_thread_num();
				
	// 			for(b = myid; b < B; b+=np) {
	// 				// printf("myid = %d b = %d\n", myid, b);
	// 				for(xy = 0; xy < X*Y - 1; ++xy) {
	// 					x = xy % X;
	// 					y = xy / X;
	// 					// printf("x = %d y = %d\n", x, y);
	// 					pool_avg_global_core11(in + b*Y*X*L + y*X*L + x*L, out_16 + b*L, L);
	// 				}	
	// 				x++;
	// 				// printf("x = %d y = %d\n", x, y);
	// 				pool_avg_global_core11_last(in + b*Y*X*L + y*X*L + x*L, out + b*L, out_16 + b*L, XY, L);
	// 			}
	// 		}
	// 	}
	//  #endif
